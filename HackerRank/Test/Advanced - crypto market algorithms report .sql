...
FROM (
    SELECT 
        c.algorithm, 
        SUM(volume) as trannsactions_Q1 FROM coins c JOIN transactions t ON c.code = t.coin_code WHERE EXTRACT(YEAR FROM t.dt) = 2020  AND QUARTER(t.dt) = 1 GROUP BY c.algorithm ORDER BY c.algorithm) as t1
JOIN (SELECT c.algorithm,
        SUM(volume) as trannsactions_Q2 FROM coins c JOIN transactions t ON c.code = t.coin_code WHERE EXTRACT(YEAR FROM t.dt) = 2020  AND QUARTER(t.dt) = 2 GROUP BY c.algorithm ORDER BY c.algorithm) as t2 ON t2.algorithm = t1.algorithm
